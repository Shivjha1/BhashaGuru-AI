...
export default async function Parent() {
  ...
  return <main className="wrap"><span className="pill">PARENT · FAMILY OVERVIEW</span><h1>Your child's learning</h1><p className="muted">Follow participation, quiz results, and progress in one place.</p>{profile?.children.length ? <div className="grid">{profile.children.map((child) => <Child key={child.userId} id={child.userId} name={child.user.name} className={child.user.className || "Class not set"} language={child.user.language} />)}</div> : <div className="card"><h3>No child linked yet</h3><p className="muted">A teacher or administrator can link a student to this parent account.</p></div>}</main>;
}

async function Child({ id, name, className, language }: { id: string; name: string; className: string; language: string }) {
  ...
  return <div className="card"><h2>👧 {name}</h2><p className="muted">{className} · {language}</p><div className="stats-grid"><div className="stat-card"><span>Average score</span><strong>{average}%</strong></div><div className="stat-card"><span>Lessons completed</span><strong>{completed}</strong></div><div className="stat-card"><span>Quiz attempts</span><strong>{attempts.length}</strong></div></div><h3>Recent participation</h3>{attempts.length ? <div className="activity-list">{attempts.map((attempt) => <p key={attempt.id}><b>{attempt.lesson.title}</b> · {attempt.score}/{attempt.total} · {attempt.weakTopic || "Strong"}</p>)}</div> : <p className="muted">No quiz activity recorded yet.</p>}</div>;
}
